...
//
// For the interface-transport tests the velocity is prescribed: the cell-centred velocity at t^n
// feeds the normal predictor (eq. 21, 23) and the face-centred velocity at
// t^{n+1/2} plays the role of U^ADV (eq. 28) — it is exactly divergence-free
// for the test flows, so no MAC projection is needed (flow.cpp adds it).
#pragma once
#include "grid.h"
#include "bc.h"
#include "advect.h"
...
// Face states phi_{i+1/2,j} (stored at x-face (i+1,j), i.e. xed(i,j) is the face between cells i-1 and i)
// and phi_{i,j+1/2} (yed(i,j) between cells j-1 and j), upwinded with the face velocities.
...
// One full step phi^{n+1} = phi^n - dt [U.grad phi]^{n+1/2}, eq. (12), with prescribed velocity.
